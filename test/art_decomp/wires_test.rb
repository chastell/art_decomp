...
module ArtDecomp
  describe Wires do
    ...
    let(:wires) do
      Wires.new([
        Wire.from_arrays([fun_a, :outs, 0, 1], [fun_b, :ins, 1, 1]),
        Wire.from_arrays([fun_a, :outs, 1, 1], [fun_b, :ins, 0, 1]),
      ])
    end

    describe '.from_array' do
      it 'constructs the Wires from a minimal Array' do
        fa = Wires.from_array([[[fun_a, :outs, 0, 1], [fun_b, :ins, 1, 1]],
                               [[fun_a, :outs, 1, 1], [fun_b, :ins, 0, 1]]])
        fa.must_equal wires
      end
    end
    ...
    describe '#+' do
      it 'sums the two Wires objects' do
        a = Wires.from_array([[[fun_a, :outs, 0, 1], [fun_b, :ins, 1, 1]]])
        b = Wires.from_array([[[fun_a, :outs, 1, 1], [fun_b, :ins, 0, 1]]])
    ...
    describe '#==' do
      it 'compares two Wires with regard to contents' do
        ...
        wires.wont_equal Wires.new([
          Wire.from_arrays([fun_a, :outs, 0, 1], [fun_b, :ins, 0, 1]),
          Wire.from_arrays([fun_a, :outs, 1, 1], [fun_b, :ins, 1, 1]),
        ])
      end
    end
