...
module ArtDecomp describe Seps do
  ...
  describe '.new' do
    it 'can take a matrix to start from' do
      Seps.new(matrix: [B[2], B[], B[0]])
        .must_equal Seps.new(blocks: [B[0,1], B[1,2]])
    end

    it 'normalises the matrix' do
      Seps.new(blocks: [B[0,1,2]]).must_equal Seps.new matrix: []
    end

    it 'builds a proper matrix' do
      Seps.new(blocks: []).must_equal Seps.new matrix: []
      Seps.new(blocks: [B[0]]).must_equal Seps.new matrix: []
      Seps.new(blocks: [B[0,1]]).must_equal Seps.new matrix: []
      Seps.new(blocks: [B[0], B[1]]).must_equal Seps.new matrix: [B[1], B[0]]
      Seps.new(blocks: [B[0], B[1], B[2,3], B[4]]).must_equal Seps.new matrix:
        [B[4,3,2,1], B[4,3,2,0], B[4,1,0], B[4,1,0], B[3,2,1,0]]
      Seps.new(blocks: [B[1], B[4]]).must_equal Seps.new matrix:
        [B[4,1], B[4,3,2,0], B[4,1], B[4,1], B[3,2,1,0]]
      Seps.new(blocks: [B[0,2,3], B[1], B[4]]).must_equal Seps.new matrix:
        [B[4,1], B[4,3,2,0], B[4,1], B[4,1], B[3,2,1,0]]
      Seps.new(blocks: [B[0,1,2,3], B[0,2,3,4]]).must_equal Seps.new matrix:
        [B[], B[4], B[], B[], B[1]]
      Seps.new(blocks: [B[0], B[1], B[2], B[3], B[4]])
        .must_equal Seps.new matrix:
        [B[4,3,2,1], B[4,3,2,0], B[4,3,1,0], B[4,2,1,0], B[3,2,1,0]]
      Seps.new(blocks: [B[0,1,2], B[1,2,3,4]]).must_equal Seps.new matrix:
        [B[4,3], B[], B[], B[0], B[0]]
    end
  end
  ...
  describe '#==' do
    it 'compares two Seps by value' do
      assert Seps.new(blocks: [B[0,1], B[1,2]]) ==
        Seps.new(blocks: [B[0,1], B[1,2]]).dup
      assert Seps.new(blocks: [B[0,1], B[1,2]]) ==
        Seps.new(blocks: [B[1,2], B[0,1]])
      refute Seps.new(blocks: [B[0,1], B[1,2]]) ==
        Seps.new(blocks: [B[0,2], B[1,2]])
    end
  end
  ...
  describe '#empty?' do
    it 'returns a predicate whether the Seps are empty' do
      Seps.new(blocks: []).must_be :empty?
      Seps.new(blocks: [B[0,1]]).must_be :empty?
      Seps.new(blocks: [B[0], B[1]]).wont_be :empty?
    end
  end

  describe '#inspect' do
    it 'returns self-initialising representation' do
      Seps.new(blocks: []).inspect.must_equal 'ArtDecomp::Seps.new matrix: []'
  ...
  describe '#size' do
    it 'returns the number of separations' do
      Seps.new(blocks: []).size.must_equal 0
      Seps.new(blocks: [B[0], B[1]]).size.must_equal 1
      Seps.new(blocks: [B[0,1,2], B[1,2,3,4]]).size.must_equal 2
      Seps.new(blocks: [B[0,2,3], B[1], B[4]]).size.must_equal 7
      Seps.new(blocks: [B[0,1,2,3], B[0,2,3,4]]).size.must_equal 1
      Seps.new(blocks: [B[0], B[1], B[2], B[3], B[4]]).size.must_equal 10
